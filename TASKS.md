...
#### [x] T-1101 `PlatformDetectionController`
**Depends on:** T-002
...
**Verified:** Classification logic split into the pure `src/shared/Formulas/PlatformDetection.lua` (`DetectPlatform`, taking plain strings/numbers/booleans instead of `Enum.UserInputType`/live services, exactly so it's testable against a scripted event stream without mocking `UserInputService` for real) + the thin `src/client/Shared/Controllers/PlatformDetectionController.lua` wrapper. `isTenFootInterface` wins outright over input type (Roblox's own console/TV signal is more authoritative than whatever key was pressed last); `Gamepad*` input types map to Console; `Touch` splits into Mobile/Tablet by the viewport's shorter dimension (a documented heuristic — Roblox has no direct "is this a tablet" API). Lives in `src/client/Shared/Controllers/` (loaded in both places per that folder's own README) since platform detection matters in the Lobby too, not just combat. "Switching input mid-session updates the signal live" (DoD) is satisfied by `UserInputService.LastInputTypeChanged`, the one live-updating signal Roblox actually fires on an input-method switch. Zero-dependency module, ran for real via `lune`: every raw input-type classifies correctly, `isTenFootInterface` overrides mid-keyboard-session, Touch splits correctly by viewport size, and a scripted 4-event input-type-changed stream (keyboard → gamepad plugged in → back to mouse → touch) produces the exact expected platform sequence — T-1101's own test-case wording, satisfied literally. `PlatformDetection.spec.lua` mirrors this; `Constants.Platform` added as a typo-safe string enum matching the formula's `Platform` type.

#### [x] T-1102 Input remap: gamepad + touch
...
**Verified:** `src/shared/Formulas/InputBindings.lua` — the single source of truth for all four platforms' bindings, consumed directly by both the coverage test AND the real input code (not two independently-maintained lists that could drift): `InputController.lua` builds its `Enum.KeyCode`/`Enum.UserInputType` dispatch tables directly from `InputBindings.Desktop`/`.Console` at module load (a gamepad button press arrives as an ordinary `KeyCode`-bearing `InputObject` in Roblox — `Enum.KeyCode.ButtonX`, etc. — so the exact same `InputBegan` handler and the exact same six Signals from T-401 serve keyboard/mouse and gamepad both, no gamepad-specific branching needed). Console assigns GDD's two explicit callouts (`ButtonR2` trigger for Ultimate, `ButtonR3` right-stick click for lock-on) plus face-button defaults for the rest (`ButtonX`/`ButtonY`/`ButtonB`/`ButtonA` — no jump/dodge mechanic exists in this game to conflict with `ButtonA`, so Dash claims it). Mobile/Tablet share one scheme (`TouchControlsUIController`, T-1104): a single Attack button plays GDD's own M1-tap/M2-hold role instead of needing two buttons, and `TargetSwitch` is marked `autoAssist = true` (GDD explicitly calls touch lock-on "auto-assist," not a manual input) — still "reachable" per the DoD, just not via a dedicated tap target; `TouchControlsUIController` fires it automatically alongside every Attack press. Zero-dependency coverage check, ran for real via `lune`: all four platforms have a bound source for all six actions, Mobile and Tablet are the identical table, and a deliberately-incomplete fixture correctly fails the check (proving it has real teeth, not vacuously true). `InputBindings.spec.lua` mirrors this.

#### [x] T-1103 Responsive UI framework
**Depends on:** T-002
...
**Verified:** `src/shared/Formulas/ResponsiveBreakpoints.lua` (pure `SelectBreakpoint`, keyed off the viewport's *shorter* dimension so it's orientation-agnostic) + `src/client/Shared/Controllers/ResponsiveUIController.lua` (applies a `UIScale` per breakpoint via `:Apply(guiObject)`, tracked in a weak-keyed table, refreshed live on `Camera.ViewportSize` changes). Only `UIScale` is used, not `AspectRatioConstraint` — every panel this applies to (`UIBuilder.CreatePanel`) is a fixed 520x420 Offset-sized frame, not a Scale-relative one, so there's no parent-shape distortion for `AspectRatioConstraint` to guard against; `UIScale` alone both shrinks-to-fit *and* preserves proportions by construction, documented in `ResponsiveUIController`'s own header rather than adding an inert constraint just to tick the letter of the description. Applied to all five named surfaces: `ShopUIController`, `LoadoutUIController`, `MapSelectController`, `PartyUIController`, `ProgressionBoardController` (the four kiosk panels + the persistent lobby HUD) — plus a genuinely new `src/client/Battlefield/Controllers/BattlefieldHUDController.lua` (HP + Ultimate Gauge bars) written specifically because no combat HUD existed anywhere yet to apply "HUD" scaling to; documented as necessary glue, not its own task id, the same framing `CombatController` already established. Caught and fixed a real bug while writing the breakpoint thresholds: the original `Medium` ceiling (1200) put 1920x1080 — the single most common desktop resolution (short dimension 1080) — into the tablet-scaled tier, needlessly shrinking the UI for the majority of desktop players; caught by actually running the reference-resolution table through the function via `lune`, not by inspection, and fixed by lowering the ceiling to 900. Ran the corrected thresholds for real via `lune` against 8 reference resolutions spanning the smallest supported phone (iPhone SE, 375x667) through a super-ultrawide desktop (5120x1440) — all land in the correct tier with `scale` always in `(0, 1]`, both portrait and landscape orientations of the same phone select the same tier, and every boundary (700/701, 900/901, 2560/2561) crosses at the exact pixel. `ResponsiveBreakpoints.spec.lua` mirrors this; the actual visual check across live reference resolutions is manual per the task's own guidance (S-1301).

#### [x] T-1104 `TouchControlsUIController`
...
**Test cases:** Manual/visual.
**Verified:** `src/client/Battlefield/Controllers/TouchControlsUIController.lua` — Attack/Special/Dash/Ultimate buttons, built and torn down entirely by `:Destroy()` (never just `Visible`/`Enabled` toggled) in response to `PlatformDetectionController.PlatformChanged`, so there is no dormant instance on Desktop/Console that could ever intercept a click — the DoD's "removed from layout," satisfied by construction rather than a property flip. The Attack button reads press duration against `Constants.UI.TouchHeavyHoldSeconds` to fire `LightAttack` (quick release) or `HeavyAttack` (held past the threshold) — mirroring GDD's own M1-tap/M2-hold framing for the desktop row — and also fires `TargetSwitch` on every press (`InputBindings.Mobile.TargetSwitch.autoAssist`, T-1102). Buttons fire `InputController`'s existing six Signals directly via `Activated`/`InputBegan`/`InputEnded`, never touching `UserInputService`, so `CombatController`/`TargetLockController` need zero touch-specific handling — the same signals, same debounce-free direct fire path either way. Manual/visual per the task's own guidance (S-1301) for the actual on-device look/feel.

#### [x] T-1105 Combat input/timing parity audit
...
**Verified — PASSED.** Grepped every `.Client:`-exposed method across `src/server/Battlefield/Services/` (the complete set of combat-adjacent Knit-generated remotes) — seven total: `DashService.Client:RequestDash(player)`, `SpecialAttackService.Client:RequestSpecial(player)`, `UltimateGaugeService.Client:RequestUltimate(player)`/`:GetGauge(player)` (read-only), `PlayerHealthService.Client:GetHealth(player)` (read-only), `MapClearService.Client:AcknowledgeResults(player)` (not combat-damage-related), and `CombatService.Client:RequestAttack(player, attackType: "Light" | "Heavy")` — the one method that takes an argument at all beyond `player`, and it's a closed two-value enum-like string, never a number. `HitboxService` and `PoiseService` expose zero `.Client` methods (confirmed via their own header comments, both already documented as server-authoritative-only back in Phase 4/T-404/T-408). Every actual damage/poise/cooldown/timing value is computed entirely server-side from `HitboxGeometry`/`ComboResolver`/`Cooldown`/`RateLimiter`/`ComboTrees` data the client never sends — input-method-agnostic by the same construction T-1102 relies on (gamepad and touch both funnel into the identical six client-side intent Signals `CombatController` already forwards as bare `"Light"`/`"Heavy"`/etc. strings, so there is no separate touch/gamepad code path that could smuggle a different kind of value through). Same audit re-confirmed this phase after T-1102's changes — `CombatController.lua` itself was untouched by T-1102 (only `InputController.lua`'s internal dispatch tables changed, its output Signals stayed identical), so parity holds by inspection, not just by re-running the same grep.
