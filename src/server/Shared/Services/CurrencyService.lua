...
function CurrencyService:AddCurrency(player: Player, currencyType: string, amount: number, reason: string): number?
	...
	local newAmount = CurrencyLedger.Add(profile.Data, currencyType, amount)
	print(("[CurrencyService] +%d %s -> %s (reason: %s, new balance: %d)"):format(amount, currencyType, player.Name, reason, newAmount))
