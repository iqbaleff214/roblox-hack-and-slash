return function()
	...
	describe("EnemyRegistry", function()
		...
		it("UpdatePosition refreshes a registered entry's position", function()
			EnemyRegistry.Register({ id = "A", tier = "FootSoldier", poiseMax = 0, position = { x = 0, y = 0, z = 0 }, takeDamage = function() end })
			EnemyRegistry.UpdatePosition("A", { x = 5, y = 0, z = 5 })
			local entry = EnemyRegistry.Get("A")
			expect((entry :: any).position.x).to.equal(5)
			expect((entry :: any).position.z).to.equal(5)
		end)

		it("UpdatePosition on an unknown id is a safe no-op", function()
			expect(function()
				EnemyRegistry.UpdatePosition("DoesNotExist", { x = 1, y = 1, z = 1 })
			end).never.to.throw()
		end)
	end)
end
